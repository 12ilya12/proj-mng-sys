...
export class StatusRepository {
  ...
  ): Promise<IPaging<StatusPersistType>> {
  ...
  async create(
    statusInfo: CreateStatusDto
  ): Promise<StatusPersistType> {
  ...
  async hasTasks(id: number): Promise<boolean> {
    ...
    taskCount = (
      await this.drizzle.db
        .select({ value: count() })
        .from(taskTable)
        .where(eq(taskTable.statusId, id))
    )[0].value
    return taskCount > 0
  }
  ...
  async delete(id: number) {
    await this.drizzle.db.transaction(async tx => {
      const statusForDelete = await this.drizzle.db
        .select()
        .from(statusTable)
        .where(eq(statusTable.id, id))
      ...
      if (await this.hasTasks(id)) {
        throw new ConflictException(
          'У данного статуса есть связанные задачи'
        )
      }
      await this.drizzle.db
        .delete(statusTable)
