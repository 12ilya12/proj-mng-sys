...
export class DependencyRepository {
  ...
  async getAll(
  ...
  ): Promise<IPaging<DependencyPersistType>> {
    let items = this.drizzle.db
      .select()
      .from(dependencyTable)
      .where(eq(dependencyTable.parentTaskId, parentTaskId))
    const totalItems: number = (await items).length
    const totalPages: number = pagingOptions.pageSize
      ? Math.ceil(totalItems / pagingOptions.pageSize)
      : 1
    ...
    if (pagingOptions.page)
      items.offset(
        (Number(pagingOptions.page) - 1) *
          Number(pagingOptions.pageSize)
      )
  ...
  async create(
    ...
    childTaskId: number,
    userInfo
  ): Promise<DependencyPersistType> {
    return await this.drizzle.db.transaction(async tx => {
      const parentTaskUserId = (
        await this.taskRepository.getById(parentTaskId)
      ).userId
      const childTaskUserId = (
        await this.taskRepository.getById(childTaskId)
      ).userId
      //Обычный пользователь может создавать зависимости только между своими задачами
      if (
        userInfo.role === 'USER' &&
        (userInfo.id !== parentTaskUserId ||
          userInfo.id !== childTaskUserId)
      )
  ...
  async delete(parentTaskId: number, dependencyId: number) {
    await this.drizzle.db.transaction(async tx => {
      const depForDelete = this.drizzle.db
        .select()
        .from(dependencyTable)
        .where(
          and(
            eq(dependencyTable.id, dependencyId),
            eq(dependencyTable.parentTaskId, parentTaskId)
          )
        )
      if ((await depForDelete).length !== 0)
        await this.drizzle.db
          .delete(dependencyTable)
          .where(eq(dependencyTable.id, dependencyId))
      else
        throw new NotFoundException(
          `Не найдена зависимость с идентификатором ${dependencyId} задачи с идентификатором ${parentTaskId}`
        )
    })
  }
}
