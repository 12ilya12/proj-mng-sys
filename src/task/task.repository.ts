...
export class TaskRepository {
  ...
  async getAll(
    ...
    userInfo
  ): Promise<IPaging<TaskPersistType>> {
  ...
  async update(
    id: number,
    taskInfo: UpdateTaskDto
  ): Promise<TaskPersistType> {
    let updatedTask = (
      await this.drizzle.db
        .update(taskTable)
        .set(taskInfo)
  ...
  async updateStatus(
    ...
    let updatedTask = (
      await this.drizzle.db
        ...
        .where(
          and(eq(taskTable.id, id), eq(taskTable.userId, userId))
        )
        .returning()
    )[0]
    return updatedTask
  ...
  async hasDependencies(id: number): Promise<boolean> {
    ...
    dependenciesCount = (
      await this.drizzle.db
        ...
        .where(
          or(
            eq(dependencyTable.parentTaskId, id),
            eq(dependencyTable.childTaskId, id)
          )
        )
    )[0].value
  ...
  async delete(id: number) {
    await this.drizzle.db.transaction(async tx => {
      const taskForDelete = await this.drizzle.db
        .select()
        .from(taskTable)
        .where(eq(taskTable.id, id))
      ...
      if (await this.hasDependencies(id)) {
        throw new ConflictException(
          'У данной задачи есть зависимости'
        )
      }
      await this.drizzle.db
        .delete(taskTable)
