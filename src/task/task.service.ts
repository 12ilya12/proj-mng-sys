...
export class TaskService {
  ...
  async create(createTaskDto: CreateTaskDto): Promise<TaskDto> {
    if (
      ...
      throw new NotFoundException(
        `Не найдена категория с идентификатором ${createTaskDto.categoryId}`
      )
    }
    if (
      (await this.statusRepository.getById(createTaskDto.statusId)) ==
      null
    ) {
      throw new NotFoundException(
        `Не найден статус с идентификатором ${createTaskDto.statusId}`
      )
    }
    if (
      (await this.userRepository.findById(createTaskDto.userId)) ==
      null
    ) {
      throw new NotFoundException(
        `Не найден пользователь с идентификатором ${createTaskDto.userId}`
      )
    }
  ...
  async update(
    ...
    if (
      updateTaskDto.categoryId != null &&
      (await this.categoryRepository.getById(
        updateTaskDto.categoryId
      )) == null
    ) {
      throw new NotFoundException(
        `Не найдена категория с идентификатором ${updateTaskDto.categoryId}`
      )
    }
    if (
      updateTaskDto.statusId != null &&
      (await this.statusRepository.getById(updateTaskDto.statusId)) ==
        null
    ) {
      throw new NotFoundException(
        `Не найден статус с идентификатором ${updateTaskDto.statusId}`
      )
    }
    if (
      updateTaskDto.userId != null &&
      (await this.userRepository.findById(updateTaskDto.userId)) ==
        null
    ) {
      throw new NotFoundException(
        `Не найден пользователь с идентификатором ${updateTaskDto.userId}`
      )
    }
    ...
    if (userInfo.role === 'ADMIN')
      if (
        updateTaskDto.categoryId == null &&
        updateTaskDto.statusId == null &&
        updateTaskDto.name == null &&
        updateTaskDto.description == null &&
        updateTaskDto.deadline == null &&
        updateTaskDto.priority == null &&
        updateTaskDto.userId == null
      ) {
        throw new BadRequestException(
          'Нет значений для обновления задачи'
        )
      }
    ...
    if (userInfo.role === 'USER') {
      if (await this.taskRepository.taskForUser(id, userInfo.id)) {
        throw new ForbiddenException(
          'Задача не связана с текущим пользователем'
        )
      }
      if (updateTaskDto.statusId == null) {
        throw new BadRequestException(
          'Пользователь может обновить только статус задачи. Требуется statusId.'
        )
      }
