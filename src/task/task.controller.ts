...
export class TaskController {
    ...
    @ApiQuery( {name: 'orderBy', required: false, description: 'Критерий сортировки ("id", "name", "description", "categoryId", "statusId", "userId", "deadline", "priority")', type: String} )
    @ApiBody( {description: 'Параметры фильтрации', type: TaskFilterDto} )
    ...
    @ApiParam( {name: 'id', required: true, description: 'Идентификатор задачи', type: Number} )
    ...
    @ApiCreatedResponse({ description: "Задача успешно создана", type: TaskDto })
    @ApiBadRequestResponse({ description: "Некорректный запрос" })
    ...
    @ApiOkResponse({ description: "Успех" })
    @ApiNotFoundResponse({ description: "Не найдена задача с заданным идентификатором" })
    @ApiConflictResponse( {description: "У данной задачи есть зависимости"} )
    @ApiBadRequestResponse({ description: "Некорректный запрос" })
    @SetMetadata('roles', ['ADMIN'])
