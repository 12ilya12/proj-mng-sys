...
export class UserService {
  ...
  async create(userDto: CreateUserDto): Promise<UserDto> {
    ...
    const createUserDtoWithHash: CreateUserDto = {
      login,
      password: hashPassword,
      fullName,
      email
    }
    const newUser: UserPersistType = await this.userRepository.create(
      createUserDtoWithHash,
      'USER'
    )
