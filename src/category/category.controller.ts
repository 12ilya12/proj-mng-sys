...
export class CategoryController {
  ...
  @ApiOperation({
    summary:
      'Получить информацию, есть ли связанные задачи с категорией по ID'
  })
  @ApiParam({
    name: 'id',
    required: true,
  ...
  @ApiConflictResponse({
    description: 'У данной категории есть связанные задачи'
  })
  @ApiBadRequestResponse({ description: 'Некорректный запрос' })
  @SetMetadata('roles', ['ADMIN'])
  ...
  @ApiOperation({
    summary:
      'Принудительно удалить категорию по ID вместе со всеми связанными задачами каскадно'
  })
  @ApiParam({
    name: 'id',
    required: true,
