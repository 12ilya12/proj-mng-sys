...
export class CategoryRepository {
  ...
  ): Promise<IPaging<CategoryPersistType>> {
  ...
  async hasTasks(id: number): Promise<boolean> {
    ...
    taskCount = (
      await this.drizzle.db
        .select({ value: count() })
        .from(taskTable)
        .where(eq(taskTable.categoryId, id))
    )[0].value
    return taskCount > 0
  }
  ...
  async delete(id: number) {
    await this.drizzle.db.transaction(async tx => {
      const categoryForDelete = await this.drizzle.db
        .select()
        .from(categoryTable)
        .where(eq(categoryTable.id, id))
      ...
      if (await this.hasTasks(id)) {
        throw new ConflictException(
          'У данной категории есть связанные задачи'
        )
      }
      await this.drizzle.db
